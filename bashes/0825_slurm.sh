...
#     training.num_epochs=500 logging.mode=offline \
#     policy.obs_encoder.random_crop=True \
#     policy.obs_encoder.crop_shape=[200,290] \
    # policy.obs_encoder.random_crop=False \
    # policy.obs_encoder.crop_shape=[240,320]


EXP=imgnoise-head-torso-nocrop-state01
python train.py --config-name=train_dexmachina_grasp_depth_2cam \
    ...
    training.num_epochs=800 logging.mode=offline \
    policy.obs_encoder.random_crop=False \
    # policy.obs_encoder.crop_shape=[200,280] task.dataset.max_train_episodes=10
